    ...
    public static function register($email) {
        if (!empty($_POST)) {
            ...
            if (self::$valid) {
                ...
                $existingSignup = $pdo->prepare("SELECT COUNT(*) FROM signupnewsletter WHERE signup_email_address='$email'");
                $existingSignup->execute();
                $data_exists = ($existingSignup->fetchColumn() > 0) ? true : false;

                if (!$data_exists) {
                    $sql = "INSERT INTO signupnewsletter (signup_email_address,signup_name, signup_date) VALUES ( :email,:name, :datetime)";
                    $q = $pdo->prepare($sql);
